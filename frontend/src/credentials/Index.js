...
export function Credentials(props) {
  ...
  const {
    value: accessKeyId,
    bind: bindAccessKeyId,
    reset: resetAccessKeyId,
  } = useInput("");
  const {
    value: secretAccessKey,
    bind: bindSecretAccessKey,
    reset: resetSecretAccessKey,
  } = useInput("");
  const { value: setupDone, setValue: setSetupDone } = useInput(false);

  const handleSubmit = async (evt) => {
    evt.preventDefault();
    await postJson("/credentials", { region, accessKeyId, secretAccessKey })
      .then((r) => {
        resetRegion();
        resetAccessKeyId();
